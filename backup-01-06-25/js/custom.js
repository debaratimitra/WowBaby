...
// Basic cart array
  ...
  // function singlePro(product, qty) {
  //   let productData = product;
  
  //   localStorage.getItem("details") && (cart = JSON.parse(localStorage.getItem("details"))); 
    
  //   // Find if product already in cart
  //     const existing = details.find(item => item.id === productData.id);
  //     if (existing) {
  //       existing.quantity += qty;
  //     } else {
  //       const data={...productData, quantity: qty}
  ...
  //   alert("Product added to details!");
  //   localStorage.setItem("details", JSON.stringify(cart)); // Save cart to localStorage
  // }
