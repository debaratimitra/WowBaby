...
function addToWish(product, qty) {
  localStorage.getItem("wish") && (wish = JSON.parse(localStorage.getItem("wish"))); // Correct: load into wish

  // Check if product already in wishlist
  const exists = wish.find(item => item.id === product.id);
  if (exists) {
  ...
  wish.push(productToAdd);
  localStorage.setItem("wish", JSON.stringify(wish)); // Save wish list
  showToast("Product added to wishlist!");
}
...
$(function() {
  $("#searchInput").keyup(function() {
    ...
    if (valThis == "") {
      $("#searchList").hide(); // Keep hidden if input is empty
      ...
      // Optional: show matching count
      var remainingResults = $('#searchList:visible').length;
